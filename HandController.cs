...
public class HandController : GenericEnemy {
	...
	// Sets an enemy to be possessed later (actual implementation handled within PossessableScript -- this is just a reference to the obj)
	...
	void ActivateAttackMenu(){
		if (Input.GetMouseButtonDown(1) && !(m_Anim.GetBool("Eat") || m_Anim.GetBool("Possessing") || m_Anim.GetBool("Grabbing"))){
			...
			if (grabbedObject != null){
				...
				PossessableScript grabbedPs = grabbedObject.GetComponent<PossessableScript>();
				if (grabbedObject.GetComponent<PossessableScript>().possessionPoint != null){
					target = grabbedObject.GetComponent<PossessableScript>().possessionPoint;
				}
				...
				if (iq < 100){
					...
					if (grabbedObject.GetComponent<PossessableScript>().edible){
	...
	void HighlightObject(){
		...
		for (int i = 0; i < possessables.Length; i++){
			GameObject p = possessables[i];
			//if (p.GetComponent<PossessableScript>() && p.GetComponent<PossessableScript>().CanPossess()){
			if (p.GetComponent<PossessableScript>() && p.GetComponent<PossessableScript>().CanInteract()){
		...
		if (grabbedObject && !m_Anim.GetBool("Possessing") && !m_Anim.GetBool("Eating")){
			...
			// If it's not a sprite, let it run a special function later
				...
				grabbedObject.gameObject.GetComponent<PossessableScript>().ActivateCustomOutline();
				//enemyStats.transform.position = new Vector3(hoveredObject.transform.position.x, hoveredObject.transform.position.y + hoveredObjectRenderer.sprite.bounds.max.y, transform.position.z);
				Vector3 tempEnemyStatsPos = new Vector3(cursor.position.x, cursor.position.y + 1f, transform.position.z);
				if (Input.mousePosition.y > Screen.height*.8f){
					tempEnemyStatsPos.y -= 2f;
			...
			enemyIQText.text = "IQ: "+grabbedObject.gameObject.GetComponent<PossessableScript>().iq;
			enemyRankText.text = "Rank: "+grabbedObject.gameObject.GetComponent<PossessableScript>().rank;
			enemyInfoText.text = grabbedObject.gameObject.GetComponent<PossessableScript>().infoText;
	...
	public void Possess(GameObject grabbedObject){
		...
		HasPossessedBefore(grabbedObject.GetComponent<GenericEnemy>().GetName());
		hoveredObjectRenderer.sprite = null;
		AddToPossessionMemory(grabbedObject);

		grabbedObject.GetComponent<PossessableScript>().Possess(gameObject, PossessedAttackMenu);
		enemyStats.SetActive(false);
	...
	// Eats an object
	// eathuman
	public void EatObject(bool overrideDist = false){
		Transform target = selectedTarget;
		NormalTime();
		SetAttackMenuStatus(false);
		if (selectedTarget != null){
			if (target.parent && target.parent.gameObject.GetComponent<PossessableScript>()){
				target = target.parent;
			}
			// Inedible objects
			if (!target.GetComponent<PossessableScript>().edible){
				return;
			}
			...
			bool canEat = false;
			if (overrideDist || Vector3.Distance(transform.position, target.position) < rangeRadius ||
			...
			target.gameObject.GetComponent<PossessableScript>().Freeze(wrapAroundPoint);
	...
	public void EatObject(Transform eatenObject, bool overrideDist = false){
		selectedTarget = eatenObject;
		EatObject(overrideDist);
	}
	...
	public void CompleteObjectEating(){
		if (selectedTarget != null){
			...
			if (selectedTarget.GetComponent<GenericEnemy>() != null){
				selectedTarget.GetComponent<GenericEnemy>().FinishBeingDevoured(gameObject);
			}
			if (selectedTarget != target && target.GetComponent<GenericEnemy>() != null){
				target.GetComponent<GenericEnemy>().FinishBeingDevoured(gameObject);
			}
	...
	void AddEXP(GameObject obj){
		exp += obj.GetComponent<PossessableScript>().exp;
	}
	...
	public void PossessHuman(){
		...
		if (selectedTarget != null){
			...
			if (!target.GetComponent<PossessableScript>().isHuman){
	...
	public void CompleteHumanPossession(){
		if (selectedTarget != null){
			...
			//target.gameObject.GetComponent<PossessableScript>().PostDramaticEffect();
