...
public class NetworkGenericEnemy : NetworkBehaviour {
	/// <summary>
	...
	public virtual void SetParanoia(float f){
		Camera.main.GetComponent<NetCameraTarget>().SetParanoia(f);
	}
	...
	public virtual void DamagedEvent(GameObject attacker){
		...
		if (attacker.GetComponent<Weapon>()){
			if (GetComponent<NetworkPossessableScript>() && GetComponent<NetworkPossessableScript>().possessed == false){
	...
	/// Note that NetworkPossessableScript calls this function, so referring back to the NetworkPossessableScript causes a Stack Overflow
	/// </summary>
	public virtual void Depossess(){
	
	}
	[Command]
	public virtual void CmdActivateAttack(){
		Camera.main.GetComponent<NetCameraTarget>().SetTimeScale(1f);
	}
	...
	public virtual void CmdDeath(){
		if (GetComponent<NetworkPossessableScript>()){
			if (GetComponent<NetworkPossessableScript>().possessed){
				GetComponent<NetworkPossessableScript>().Depossess();
			}
		}
	}
	...
	public void AddSoundRing(Vector3 location, float volume){
		Camera.main.GetComponent<NetCameraTarget>().AddSoundRing(location, volume, transform);
	}
