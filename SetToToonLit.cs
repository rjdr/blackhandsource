...
public class SetToToonLit : MonoBehaviour {
	public bool setChildrenToToon = true;
	// Sets an object to use a toon shader without creating a new material
	void Start () {
		if (gameObject.GetComponent<MeshRenderer>()){
			gameObject.GetComponent<MeshRenderer>().material.shader = Shader.Find("Toon/Lit");
		}
	...
	void RecursiveToon(Transform trans){
		foreach (Transform t in trans){
			if (t.GetComponent<MeshRenderer>()){
				t.GetComponent<MeshRenderer>().material.shader = Shader.Find("Toon/Lit");
			}
			RecursiveToon(t);
		}
	}
